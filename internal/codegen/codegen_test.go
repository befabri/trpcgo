...
func TestWriteAppRouterNoProcs(t *testing.T) {
	...
	// Without procedures there is no router preamble at all: a package-rooted
	// contract need not depend on tRPC.
	if strings.Contains(output, "export type AppRouter =") {
		t.Errorf("AppRouter should not be emitted with no procedures.\nOutput:\n%s", output)
	}
	if strings.Contains(output, "@trpc/server") {
		t.Errorf("@trpc/server should not be imported with no procedures.\nOutput:\n%s", output)
	}
}
...
func TestWriteAppRouterEmitsDefinitionsWithoutProcs(t *testing.T) {
	defs := []typemap.TypeDef{
		{
			Name:    "Heartbeat",
			Kind:    typemap.TypeDefInterface,
			Comment: "Heartbeat is pushed, never requested.",
			Fields: []typemap.Field{
				{Name: "runner", Type: "string", GoKind: "string"},
			},
		},
	}

	var buf bytes.Buffer
	if err := codegen.WriteAppRouter(&buf, nil, defs); err != nil {
		t.Fatal(err)
	}
	output := buf.String()
	...
	for _, want := range []string{
		...
		"/** Heartbeat is pushed, never requested. */",
	...
	if strings.Contains(output, "@trpc/server") {
		t.Errorf("definitions alone must not import @trpc/server.\nOutput:\n%s", output)
	}
}

func TestWriteZodSchemasRootsJoinProcedureInputs(t *testing.T) {
	procs := []codegen.ProcEntry{
		{Path: "job.create", ProcType: "mutation", InputTS: "CreateJobInput", OutputTS: "Job"},
	}
	defs := []typemap.TypeDef{
		{
			Name:   "CreateJobInput",
			Kind:   typemap.TypeDefInterface,
			Fields: []typemap.Field{{Name: "name", Type: "string", GoKind: "string"}},
		},
		{
			Name:   "Heartbeat",
			Kind:   typemap.TypeDefInterface,
			Fields: []typemap.Field{{Name: "pending", Type: "number", GoKind: "int"}},
		},
	}

	// Without roots, a type no procedure takes as input gets no schema.
	var inputsOnly bytes.Buffer
	if err := codegen.WriteZodSchemas(&inputsOnly, procs, defs, typemap.ZodStandard); err != nil {
		t.Fatal(err)
	}
	if strings.Contains(inputsOnly.String(), "HeartbeatSchema") {
		t.Errorf("Heartbeat should have no schema without roots.\nOutput:\n%s", inputsOnly.String())
	}

	var both bytes.Buffer
	if err := codegen.WriteZodSchemas(&both, procs, defs, typemap.ZodStandard, codegen.ZodOptions{Roots: []string{"Heartbeat"}}); err != nil {
		t.Fatal(err)
	}
	output := both.String()
	for _, want := range []string{"export const CreateJobInputSchema", "export const HeartbeatSchema"} {
		if !strings.Contains(output, want) {
			t.Errorf("output missing %q.\nOutput:\n%s", want, output)
		}
	}
