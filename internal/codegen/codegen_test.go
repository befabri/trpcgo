...
func TestORPCImportStatement(t *testing.T) {
	output := generateORPCFromFixture(t, "basic")

	t.Run("imports from @orpc/client", func(t *testing.T) {
		if !strings.Contains(output, "from '@orpc/client';") {
			t.Fatalf("output missing @orpc/client import.\nOutput:\n%s", output)
		}
	})

	t.Run("imports expected types", func(t *testing.T) {
		// Extract the import block to avoid matching types elsewhere (e.g. RouterClient).
		start := strings.Index(output, "import type {")
		end := strings.Index(output, "} from '@orpc/client';")
		if start == -1 || end == -1 {
			t.Fatalf("could not find import block.\nOutput:\n%s", output)
		}
		importBlock := output[start:end]
		for _, typ := range []string{"Client", "InferClientInputs", "InferClientOutputs"} {
			if !strings.Contains(importBlock, typ) {
				t.Errorf("import missing %s.\nImport block:\n%s", typ, importBlock)
			}
		}
	})
...
func TestORPCHelperTypes(t *testing.T) {
	output := generateORPCFromFixture(t, "basic")

	t.Run("$Proc helper emitted", func(t *testing.T) {
		if !strings.Contains(output, "type $Proc<TInput, TOutput> = Client<") {
			t.Errorf("output missing $Proc helper type.\nOutput:\n%s", output)
		}
	})

	t.Run("$Sub helper emitted", func(t *testing.T) {
		if !strings.Contains(output, "type $Sub<TInput, TOutput> = Client<") {
			t.Errorf("output missing $Sub helper type.\nOutput:\n%s", output)
		}
	})

	t.Run("$Sub uses AsyncIteratorObject", func(t *testing.T) {
		if !strings.Contains(output, "AsyncIteratorObject<TOutput>") {
			t.Errorf("$Sub should wrap output in AsyncIteratorObject.\nOutput:\n%s", output)
		}
	})
}

func TestORPCRouterInputsOutputs(t *testing.T) {
	output := generateORPCFromFixture(t, "basic")

	t.Run("RouterInputs uses InferClientInputs", func(t *testing.T) {
		if !strings.Contains(output, "export type RouterInputs = InferClientInputs<RouterClient>;") {
			t.Fatalf("output missing RouterInputs.\nOutput:\n%s", output)
		}
	})

	t.Run("RouterOutputs uses InferClientOutputs", func(t *testing.T) {
		if !strings.Contains(output, "export type RouterOutputs = InferClientOutputs<RouterClient>;") {
			t.Fatalf("output missing RouterOutputs.\nOutput:\n%s", output)
		}
	})

	t.Run("RouterClient nests procedures under namespace", func(t *testing.T) {
		idx := strings.Index(output, "export type RouterClient = {")
		if idx == -1 {
			t.Fatalf("missing RouterClient.\nOutput:\n%s", output)
		}
		tree := output[idx:]
		...
		for _, proc := range []string{"getById:", "listUsers:", "createUser:", "onCreated:"} {
			if !strings.Contains(tree, proc) {
				t.Errorf("missing procedure key %s in RouterClient.\nTree:\n%s", proc, tree)
			}
		}
	})
}

func TestGenerateORPCEnhanced(t *testing.T) {
	output := generateORPCFromFixture(t, "enhanced")

	t.Run("union type Status", func(t *testing.T) {
		if !strings.Contains(output, "export type Status = ") {
			t.Fatalf("missing Status union type.\nOutput:\n%s", output)
		}
		for line := range strings.SplitSeq(output, "\n") {
	...
	t.Run("JSDoc on type", func(t *testing.T) {
		idx := strings.Index(output, "User represents a registered user.")
		ifaceIdx := strings.Index(output, "export interface User {")
		if idx == -1 || ifaceIdx == -1 || idx > ifaceIdx {
			t.Errorf("JSDoc should appear before User interface.\nOutput:\n%s", output)
		}
	})
	...
	t.Run("JSDoc on field", func(t *testing.T) {
		...
		if !strings.Contains(output[commentIdx:], "id: string;") {
			t.Errorf("id field not found after JSDoc comment.\nOutput:\n%s", output)
		}
	})
	...
	t.Run("User interface emitted once", func(t *testing.T) {
		count := strings.Count(output, "export interface User {")
		if count != 1 {
			t.Errorf("User interface emitted %d times, want 1.\nOutput:\n%s", count, output)
		}
	...
	t.Run("oRPC procedure types", func(t *testing.T) {
		if !strings.Contains(output, "$Proc<GetUserInput, User>") {
			t.Errorf("missing query as $Proc.\nOutput:\n%s", output)
		}
		if !strings.Contains(output, "$Proc<void, Paginated<User>>") {
			t.Errorf("missing void query with generic output.\nOutput:\n%s", output)
		}
		if !strings.Contains(output, "$Proc<CreateUserInput, User>") {
			t.Errorf("missing mutation as $Proc.\nOutput:\n%s", output)
		}
	})

	t.Run("idempotent generation", func(t *testing.T) {
		second := generateORPCFromFixture(t, "enhanced")
		if output != second {
			t.Error("generating twice should produce identical output")
		}
	})
...
func TestGenerateORPCEnhancedDiveArray(t *testing.T) {
	output := generateORPCFromFixture(t, "enhanced")
...
func TestWriteORPCRouterOnlyEmitsUsedProcTypes(t *testing.T) {
	procs := []codegen.ProcEntry{
		{Path: "user.get", ProcType: "query", InputTS: "string", OutputTS: "User"},
	}
	var buf bytes.Buffer
	if err := codegen.WriteORPCRouter(&buf, procs, nil); err != nil {
		t.Fatal(err)
	}
	output := buf.String()

	// $Proc should be used for the query.
	if !strings.Contains(output, "$Proc<string, User>") {
		t.Errorf("should emit $Proc for query.\nOutput:\n%s", output)
	}
	// $Sub should be defined but not used in the tree.
	if !strings.Contains(output, "type $Sub<") {
		t.Errorf("$Sub helper should always be defined.\nOutput:\n%s", output)
	}
	// Tree should not contain $Sub usage.
	idx := strings.Index(output, "export type RouterClient =")
	if idx == -1 {
		t.Fatalf("missing RouterClient.\nOutput:\n%s", output)
	}
	...
	if strings.Contains(tree, "$Sub<") {
		t.Errorf("$Sub should not appear in tree for query-only router.\nOutput:\n%s", output)
	}
}

func TestWriteORPCRouterSubscriptionOnly(t *testing.T) {
	procs := []codegen.ProcEntry{
		{Path: "events.onMessage", ProcType: "subscription", InputTS: "void", OutputTS: "Message"},
	}
	var buf bytes.Buffer
	...
	if !strings.Contains(output, "$Sub<void, Message>") {
		t.Errorf("should emit $Sub for subscription.\nOutput:\n%s", output)
	}
	// $Proc should still be emitted (it's always defined), but not used in the tree.
	if !strings.Contains(output, "type $Proc<") {
		t.Errorf("$Proc helper should always be emitted.\nOutput:\n%s", output)
	}
}

func TestWriteORPCRouterNoProcs(t *testing.T) {
	var buf bytes.Buffer
	if err := codegen.WriteORPCRouter(&buf, nil, nil); err != nil {
		t.Fatal(err)
	}
	output := buf.String()

	// Inference helpers should not be imported or emitted with no procedures.
	for _, typ := range []string{"InferClientInputs", "InferClientOutputs", "RouterInputs", "RouterOutputs"} {
		if strings.Contains(output, typ) {
			t.Errorf("%s should not appear with no procedures.\nOutput:\n%s", typ, output)
		}
	}
	if !strings.Contains(output, "export type RouterClient =") {
		t.Errorf("RouterClient should always be emitted.\nOutput:\n%s", output)
	}
}

func TestORPCPropertyQuotingInTreeNodes(t *testing.T) {
	procs := []codegen.ProcEntry{
		{Path: "my-ns.get", ProcType: "query", InputTS: "void", OutputTS: "string"},
	}
...
func TestORPCLeafAndNamespaceCollision(t *testing.T) {
	procs := []codegen.ProcEntry{
		{Path: "user", ProcType: "mutation", InputTS: "string", OutputTS: "string"},
		{Path: "user.get", ProcType: "query", InputTS: "void", OutputTS: "User"},
	}
	...
	if !strings.Contains(output, "$Proc<string, string> & {") {
		t.Errorf("leaf+namespace should emit intersection type.\nOutput:\n%s", output)
	}
	if !strings.Contains(output, "$Proc<void, User>") {
		t.Errorf("child procedure should be emitted within namespace.\nOutput:\n%s", output)
	}
}
