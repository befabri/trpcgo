...
func WriteAppRouter(w io.Writer, procs []ProcEntry, defs []typemap.TypeDef) error {
	...
	if len(usedTypes) > 0 {
		_, _ = fmt.Fprint(w, `// Structural procedure types compatible with @trpc/client
...
`)
		if usedTypes["query"] {
			_, _ = fmt.Fprintln(w, `type $Query<TInput, TOutput> = $Procedure<"query", TInput, TOutput>;`)
		}
		if usedTypes["mutation"] {
			_, _ = fmt.Fprintln(w, `type $Mutation<TInput, TOutput> = $Procedure<"mutation", TInput, TOutput>;`)
		}
		if usedTypes["subscription"] {
			_, _ = fmt.Fprintln(w, `type $Subscription<TInput, TOutput> = $Procedure<"subscription", TInput, TOutput>;`)
		}
		_, _ = fmt.Fprintln(w)
	}
	...
	_, _ = fmt.Fprint(w, "// Router record matching tRPC's internal structure\ntype AppRouterRecord = ")
	...
	// AppRouter.
	_, _ = fmt.Fprint(w, `export type AppRouter = {
  _def: {
    _config: {
      $types: {
        ctx: object;
...
func writeRouterIO(w io.Writer, procs []ProcEntry) {
	...
	_, _ = fmt.Fprint(w, "export type RouterInputs = ")
	writeTreeNodes(w, inputTree, 0, identity)
	_, _ = fmt.Fprintln(w, ";")
	_, _ = fmt.Fprintln(w)

	_, _ = fmt.Fprint(w, "export type RouterOutputs = ")
