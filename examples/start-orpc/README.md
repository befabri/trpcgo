...
Go server + TanStack Start frontend using `@orpc/client` and `@orpc/tanstack-query`.
...
- **Subscriptions**: live feed via `experimental_streamedOptions` (SSE)
...
- **Type safety**: Go types generate TypeScript `RouterClient` for `@orpc/client`
- **Loaders**: TanStack Router loaders pre-fetch data with `ensureQueryData`
- **Cache invalidation**: `orpc.user.key()` for precise query key management
